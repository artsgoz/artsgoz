...
import { YellowCardColumns } from '../../../features/yellow-card/components/YellowCardColumns.js';
import { INITIAL_SUBJECTS } from '../../../features/yellow-card/constants.js';
import type { YellowCardSubject } from '../../../features/yellow-card/types.js';
  ...
  return (
    <div className="w-full min-h-screen bg-white flex flex-col lg:flex-row select-none text-black font-[ChulaCharasNew]">
      ...
      <aside className="w-full lg:w-[302px] shrink-0 border-r border-[#D0D0D1] bg-white flex flex-col p-5 gap-4 h-full lg:h-screen lg:sticky lg:top-0 z-10">
          ...
          {MOCK_ADVISOR_STUDENTS.map((student) => {
            ...
            return (
              ...
              >
                ...
                {/* Status Column */}
                {isSubmitted ? (
                  // Approved / Submitted State (Node 6530:25136): Green text "ส่งใบเหลืองแล้ว"
                  <span className="font-serif text-[14px] font-bold text-[#64A93C] whitespace-nowrap">
                    ส่งใบเหลืองแล้ว
                  </span>
                ) : (
                  // Initial or Checked State (Nodes 6491:23133 & 6530:26007): Checkbox icon
                  <div
        ...
        {/* Bottom Sidebar Action Buttons (Figma Nodes #6507:24197 & #6507:24207) */}
        <div className="flex flex-col gap-2.5 w-full pt-3 border-t border-[#D0D0D1] mt-auto shrink-0">
          {/* Primary Action Button (Node 6507:24197 - bg-[#E992B4]) */}
          <button
            type="button"
          ...
          {/* Logout Button (Node 6507:24207 - bg-[#F5CDDC]) */}
          <button
            type="button"
      ...
      <main className="flex-1 p-6 lg:p-12 overflow-y-auto max-w-[1100px] mx-auto flex flex-col gap-10">
        ...
        {/* Academic Grade / GPA Matrix Table (Figma Spec #6491:23450) */}
          ...
          <table className="w-full text-center border-collapse font-serif text-[16px]">
            ...
            <thead>
              <tr className="text-white font-bold text-[18px] h-[56px]">
                ...
                <th className="bg-[#E992B4] border-r border-[#404041] px-3 py-3 w-[70px]">
                  CA
                </th>
                <th className="bg-[#E992B4] border-r border-[#404041] px-3 py-3 w-[70px]">
                  CG
                ...
                <th className="bg-[#D0D0D1] text-black px-4 py-3 min-w-[160px]">
                  หมายเหตุ
                </th>
              </tr>
            </thead>
            ...
            <tbody>
              {MOCK_SEMESTERS.map((sem, index) => (
                <tr key={index} className="border-t border-[#404041] hover:bg-gray-50/80 transition-colors h-[48px]">
                  <td className="border-r border-[#404041] px-4 py-2.5 text-left font-normal text-black">
                    {sem.term}
                  </td>
                  <td className="border-r border-[#404041] px-3 py-2.5 text-black">
                    {sem.ca}
                  </td>
                  <td className="border-r border-[#404041] px-3 py-2.5 text-black">
                    {sem.cg}
