...
import { Search, Bookmark, Star, ChevronDown, Quote } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { Footer } from '../../../components/Footer/index.js';
import { MOCK_INTERNSHIP_REVIEWS, LANGUAGE_FILTER_TAGS } from '../internshipsData.js';
    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(star => (
        <Star
          key={star}
          size={14}
          className={star <= rating ? 'text-[#F8C135]' : 'text-gray-300'}
          fill={star <= rating ? '#F8C135' : 'none'}
        />
      ))}
    </div>
  );
...
export default function InternshipReviewPage() {
  ...
  const filteredReviews = useMemo(() => {
    let list = MOCK_INTERNSHIP_REVIEWS.filter(item => {
      const matchesSearch =
        !searchQuery ||
            ...
            {filteredReviews.length > 0 ? (
              filteredReviews.map(item => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 8 }}
                  className="w-full bg-white border border-[#E6E6E6] rounded-[15px] shadow-[0_4px_4px_rgba(0,0,0,0.25)] flex items-start p-4 md:p-6 gap-4 relative hover:border-[#DE5D8F]/30 transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-2">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="flex flex-col gap-0.5 min-w-0">
                        ...
                        <p className="text-[15px] md:text-[17px] font-bold text-[#636363] truncate">
                          {t(item.companyKey)}
                        </p>
                      </div>
                    ...
                    {/* Review Excerpt */}
                    <div className="bg-[#F7F8F9] rounded-[10px] px-4 py-3 relative">
                      <Quote size={16} className="text-[#DE5D8F] mb-1 opacity-60" />
                      <p className="text-[13px] md:text-[15px] text-[#404041] leading-relaxed line-clamp-3">
                        {t(item.excerptKey)}
                      </p>
                    </div>

                    {/* Author & Date */}
                    <div className="flex items-center justify-between text-[12px] md:text-[13px] text-[#8B8B8C] mt-1">
                      <span>{t(item.authorKey)}</span>
                  ...
                  <button
                    ...
                    className={`w-9 h-9 rounded-[8px] flex items-center justify-center transition-colors cursor-pointer border-none shrink-0 ${
