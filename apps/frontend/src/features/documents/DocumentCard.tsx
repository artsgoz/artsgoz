...
export function DocumentCard({
  ...
  const renderStatusBadge = () => {
    switch (doc.status) {
      case 'failed':
        return (
          <div
            className="flex items-center justify-center bg-[#F59AAE] text-[#81132B] px-3 py-1 rounded-full shrink-0"
            style={{ width: '96px', height: '38px', gap: '6px' }}
          >
            <XCircle size={14} className="shrink-0" />
            <span className="text-[14px] font-normal font-[ChulaCharasNew] leading-none">
              มีปัญหา
            </span>
          </div>
        );
      case 'pending':
        return (
          <div
            className="flex items-center justify-center bg-[#FDECC0] text-[#E2B030] px-3 py-1 rounded-full shrink-0"
            style={{ width: '96px', height: '38px', gap: '6px' }}
          >
      ...
      case 'neutral':
        return (
          <div
            className="flex items-center justify-center bg-[#D0D0D1] text-[#6D6D6D] px-3 py-1 rounded-full shrink-0"
            style={{ width: '96px', height: '38px', gap: '6px' }}
          >
  ...
  return (
    <div
      className="w-full max-w-[908px] bg-white border border-[#D0D0D1]/20 shadow-sm flex items-center justify-between p-4 md:p-6 h-[72px] md:h-[106px] rounded-[12px] transition-all duration-200 hover:shadow-md hover:border-[#DE5D8F]/20"
    >
      ...
      <div className="flex items-center flex-1 min-w-0 gap-3 md:gap-8">
        ...
        >
          {/* Desktop size: 24, Mobile size: 12 */}
      ...
      <button
        ...
        className="ml-4 flex items-center justify-center rounded-[8px] transition-colors duration-200 w-8 h-8 shrink-0 cursor-pointer"
        style={{
          backgroundColor: isBookmarked ? '#E992B4' : '#F7F8F9',
          color: isBookmarked ? '#FFFFFF' : '#E992B4',
        }}
        aria-label={isBookmarked ? 'ยกเลิกการบันทึก' : 'บันทึกเอกสาร'}
      >
        <Bookmark
          size={16}
          className="shrink-0"
